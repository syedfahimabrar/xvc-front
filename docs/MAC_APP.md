...
| Rebuilding on reconfiguration | **Restarting an engine posts its own configuration change.** A naive handler rebuilds forever — observed as alternating "playout/capture reconfigured" lines, each resetting the jitter buffer and cutting the outgoing audio. Coalesce notifications and ignore the echo of your own restart — including the one the *initial* start posts, or every launch begins with a pointless rebuild. |
| Rebuilding by reusing engines | **Recreate the `AVAudioEngine` instances; do not restart the old ones.** A stopped engine reconnected after a device change can come back with its input node delivering pure zeros: pipeline stats look healthy (silence converts to silence), the far end hears nothing, no API errors. Observed on the first real call — "worked for one hello, then nothing". |
| Silent-failure debugging | **Log a mic level meter.** Two consecutive bugs (input hijacked by the virtual device; input dead after rebuild) were invisible because every pipeline stat stays green when the input is zeros. A peak-level column in the rolling log makes a dead mic impossible to miss. |
| Rebuild scope | **Rebuild only the engine that changed.** The far-end app reads XVC Mic continuously; tearing down the playout engine because the *microphone* reconfigured yanks the stream out from under that reader, and some readers never recover — observed as "one hello, then permanent silence" on a real call while every client stat stayed green. |
| Notifications are not enough | **`AVAudioEngineConfigurationChange` does not always fire.** A nominal sample-rate change on the pinned mic (reproduced: 96 kHz → 48 kHz) made the capture engine deliver pure zeros with no notification and no error. Run a watchdog on the input signal itself: no capture callbacks for >1.5 s, or exact zeros for >3 s (a real mic's noise floor is ~0.02; zeros mean a dead engine), forces a capture rebuild with exponential backoff. Verified: detection in 2.1 s, one 2 s dropout instead of permanent silence. |
